...
public class ManageMain extends JFrame implements ActionListener 
{
  ...
  public JLabel lblpun, lblpnme, lblpedate, lblptime, lblphigh, lblplvl, lblpshare, lblwnme, lblwrank, lblnofp, lblnofw, lblnofhsp, lblnofhrw, lblpstatus, lblwun, lblhead, lblfoot, lbllogname, lbllogpass, lblnull,lblwstatus ;
  ...
  public ManageMain(){
	  ...
	  lblnofp = new JLabel();
  	  ...
  	  lblpstatus = new JLabel();
          lblwstatus = new JLabel();
	  ...
	  pnlCheck.add(lblpstatus);
	   pnlCheck.add(lblwstatus);
  ...
  public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == menucheck)
		{
	        ...
	        try{
	            ...
	            Connection connect=DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres", "postgres", "123456");
	            Statement state=connect.createStatement();
	            //sql statement to check all prisoners that are in prison
	            String sql="select * from prisoner where pstatus='inprison'";
	    ...
	    try{
	        ...
	        Connection connect=DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres", "postgres", "123456");
	        Statement state=connect.createStatement();
	        String sql="select * from warden where wstatus='working'";
	        ResultSet rs=state.executeQuery(sql);
	        while(rs.next())
	        {
	        	count[2]=count[2]+1;
	    ...
	    lblpstatus.setText("The prison must have " + count[0]/5 + " low rank warden and " +count[1]*2/4+" high rank warden.");
			 ...
			 try
	           { 
				   ...
				   */
	               ...
	               String sql = "UPDATE prisoner SET pstatus='deleted' WHERE pun="+un;
	               state.execute(sql);
	               state.close();
	               connect.close();
	               clear();
			...
			try
	           {
				/*
				   ...
				   */
	               ...
	               String sql1="select * from prisoner where pun=" +un+" AND pstatus='inprison'";
	               //sql statement to release desired prisoner
	               String sql2 = "UPDATE prisoner SET pstatus='released' WHERE pun="+un;
		...
		else if (e.getSource() == btnpdsp)
		{
			try{
				...
				//parsing unique number to integer variable
	               ...
	               String sql="select * from prisoner where pun=" +un+" AND pstatus='inprison'";
	               ResultSet rs=state.executeQuery(sql);
		...
		else if (e.getSource() == btnwadd)
		{
			try
            {
                ...
                String sql = "insert into warden (wun, wnme, wrank, wstatus) values ('" + txtwun.getText() + "','" + txtwnme.getText() + "','" + txtwrank.getText() + "','working')";
			...
			try
	           {
				   ...
				   */
	               ...
	               String sql = "UPDATE warden SET wstatus='deleted' WHERE wun ="+Integer.parseInt(txtwun.getText());
  ...
  public static void createNewTable1() {
        // SQLite connection string          
        // SQL statement for creating a new table
        String sql = "CREATE TABLE IF NOT EXISTS  prisoner(\n"
                + "	pun integer PRIMARY KEY,\n"
                + "	pnme text NOT NULL,\n"
                + "	pedate varchar(15),\n"
               + "   ptime integer,\n"
               +  "   phigh varchar,\n"
	...
	+ " pshare varchar,\n"
        ...
        try (Connection connect = DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres", "postgres", "123456");
...
public static void createNewTable2() {
        // SQLite connection string          
        // SQL statement for creating a new table
       String sql="CREATE TABLE IF NOT EXISTS warden(\n"
+ " wun integer,\n" 
+ "wnme varchar,\n" 
+ "  wrank integer,\n "
+ " pno integer,\n " 
